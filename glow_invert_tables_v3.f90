...
! nc      number of component production terms for each emission
  ...
  real,allocatable :: pedcond(:), hallcond(:) ! Pedersen and Hall conductivities in S/m (mho)
...
!------------
  ...
  real, parameter :: ecmin = 2.5, ecmax = 25000.0, ecc = 80.0
!   integer, parameter :: nec = 99, nq = 101
  ! integer, parameter :: nec0 = 200, nec1 = 150, nec2 = 46
!   integer, parameter :: nec0 = 50, nec1 = 38, nec2 = 11
  integer :: iec, iq, iut
  ...
  ! Make the vectors
  DO iec = 1, nec, 1
      ecvec( iec ) = ecmin + ( ecmax - ecmin ) * ( 1 - exp( (iec - 1) / ecc ) ) / ( 1 - exp( (nec - 1) / ecc ) )
  ENDDO
  ! DO iec = 1, nec0, 1
...
! Calculate local solar time:
  ...
  !
  do iec = 1, nec, 1
     write(6, '(A, I3, A, I3, A, I3, A, A)', advance='no') 'Progress: ', iec, ' / ', nec, ' (', INT(100.*iec/nec), '%)', char(13)
